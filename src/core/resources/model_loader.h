...
static void dumpModel(Model *model, u32 verbosity) {
	auto bb = model->mBBox;
...
struct ModelLoader {
	...
	std::unique_ptr<ModelData> load(const std::string &path)
	{
		return load(path.c_str());
	}

	std::unique_ptr<ModelData> load(const char *path) {
		scene = aiImportFile(path, aiProcessPreset_TargetRealtime_MaxQuality);
		if (scene) {
			fprintf(stdout, "Loaded file '%s'.\n", path);

			std::unique_ptr<ModelData> data(new ModelData);
			bool ret = prepareData(data.get());
			assert(ret == true && "Failed to load data");
	...
	void unload(ModelData *data) {
		releaseData(data);
	}

private:

	void releaseData(ModelData *data) {
		if (data->pModels) {
			for (auto i = u32{0}; i < data->uiModelCount; ++i) {
				auto *model = data->pModels + i;
			...
			delete data->pModels;
			data->pModels = nullptr;
			data->uiModelCount = 0;
		}

		if (data->pMaterials) {
			for (auto i = u32{0}; i < data->uiMaterialCount; ++i) {
				auto *mat = data->pMaterials + i;
			...
			data->pMaterials = nullptr;
			data->uiMaterialCount = 0;
		}
	}
	...
	bool prepareData(ModelData *data) {
	...
	void loadMaterials(ModelData *data) {
		assert(scene && "Invalid scene");
		if (scene->HasMaterials()) {
			data->pMaterials = new Material[scene->mNumMaterials];
			data->uiMaterialCount = loadMaterials(&scene->mMaterials[0], &data->pMaterials[0], scene->mNumMaterials);
			assert(data->uiMaterialCount == scene->mNumMaterials && "Failed to load all materials");

			fprintf(stdout, "Materials:\n");
			for (auto i = u32{0}; i < data->uiMaterialCount; ++i) {
				const auto *mat = &data->pMaterials[i];
				fprintf(stdout, " %d: %s\n", i, mat->mpName);
